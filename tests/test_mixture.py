...
class TestFitRobust3ComponentSmart:
    ...
    def test_custom_spec(self):
        ...
        custom_spec = BetaBinomialMixtureSpec(
            bounds_mu=[[0.05, 0.2], [0.4, 0.6], [0.8, 0.95]],
            weight_prior=2.0
        )
        
        result = fit_robust_3_component_smart(alt, depth, spec=custom_spec, random_state=42)
        
        # Check mu values respect custom bounds
        assert custom_spec.bounds_mu[0, 0] <= result["mu"][0] <= custom_spec.bounds_mu[0, 1]
        assert custom_spec.bounds_mu[1, 0] <= result["mu"][1] <= custom_spec.bounds_mu[1, 1]
        assert custom_spec.bounds_mu[2, 0] <= result["mu"][2] <= custom_spec.bounds_mu[2, 1]
    ...
    def test_n_restarts_parameter(self):
        ...
        result_1 = fit_robust_3_component_smart(alt, depth, n_restarts=1, random_state=42)
        result_5 = fit_robust_3_component_smart(alt, depth, n_restarts=5, random_state=42)
